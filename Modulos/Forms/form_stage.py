...
def create_form_stage (form_dict_data: dict) -> dict:
    ...
    form["button_wish"] = ButtonImageSound(x=200, y=310, text="",width=90, height=90, screen=form.get("screen"), image_path=var.ICON_STAR, sound_path=var.SONIDO_DESEO_IN, font_size=40, on_click=opt.cambiar_pantalla, on_click_param="form_wish")
    ...
    form["bonus_widget_list"] = [form.get("button_wish")]
...
def update_wish_widget (form_dict_data: dict):
    if form_dict_data.get("wish_disponible"):
        for widget in form_dict_data.get("bonus_widget_list"):
            widget.update()
